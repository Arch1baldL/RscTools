...
### Module Structure
- `run_*` (workflow helpers): 接受 Seurat 对象（或列表），进行标准化分析步骤；允许最小副作用（必要时设置/恢复 assay，临时元数据需清理）；通过 `message()/warning()/stop()` 反馈进度与错误；返回 Seurat 对象或同名列表。
- `plot_*` (作图美化): 只读不改 `obj`；参数以列名/数值控制外观；用 `Seurat::FetchData()` 抽取坐标/元数据；统一使用 `rlang::.data` 避免 NSE NOTE；优先 `theme_void()/coord_fixed()` 保障版式一致性。
