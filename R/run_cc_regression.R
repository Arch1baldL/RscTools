...
run_cc_regression <- function(obj, species = "mouse") {
  ...
  if (species_lower == "mouse") {
    ...
    message(sprintf("%s[run_cc_regression] Using mouse gene symbols.", get_icon("step")))
  } else if (species_lower == "human") {
    s_genes <- Seurat::cc.genes.updated.2019$s.genes
    g2m_genes <- Seurat::cc.genes.updated.2019$g2m.genes
    message(sprintf("%s[run_cc_regression] Using human gene symbols.", get_icon("step")))
  } else {
    stop(sprintf("%s[run_cc_regression] Unsupported species '%s'.", get_icon("error"), species), call. = FALSE)
  }
  ...
  if (length(check_overlap) == 0) {
    stop(
      paste0(
        sprintf("%s[run_cc_regression] No cell cycle genes found in the Seurat object.\n", get_icon("error")),
        "Please check if rownames(obj) are gene symbols (e.g., Mki67) or Ensembl IDs."
      ),
      call. = FALSE
    )
  }
  message(sprintf("%s[run_cc_regression] Found %d cell cycle genes. Proceeding...", get_icon("step"), length(check_overlap)))
  ...
  if (exists("run_v5_test")) {
    ...
    if (!is.null(v5_res) && is.list(v5_res) && !is.null(v5_res[["RNA"]])) {
      ...
      if (has_v5_layers) {
        stop(
          paste0(
            sprintf("%s[run_cc_regression] Detected Seurat v5 fragmented layers in RNA assay.\n", get_icon("error")),
            "Please run: run_v5_test(obj, assays = 'RNA', join_layers = TRUE) before run_cc_regression()."
          ),
          call. = FALSE
        )
      }
  ...
  # --- 4. 强制归一化 ---
  message(sprintf("%s[run_cc_regression] Normalizing RNA (LogNormalize) for scoring...", get_icon("step")))
  obj <- Seurat::NormalizeData(obj, assay = "RNA", normalization.method = "LogNormalize", verbose = FALSE)

  # --- 5. 细胞周期评分 ---
  message(sprintf("%s[run_cc_regression] Scoring cell cycle...", get_icon("step")))
  ...
  # --- 6. SCTransform 回归 ---
  message(sprintf("%s[run_cc_regression] Running SCTransform (regressing S.Score & G2M.Score)...", get_icon("step")))
  ...
  message(sprintf("%s[run_cc_regression] Done.", get_icon("completed")))
  return(obj)
}
